...
interface OrbitProjectProps {
  project: ProjectCaseStudy;
  className?: string;
}

/**
 * Exemplary Act III project — tall scroll runway.
 * Animation owned by createProjectsScene (transforms / stroke / textContent).
 */
export const OrbitProject = forwardRef<HTMLElement, OrbitProjectProps>(
  function OrbitProject({ project, className }, ref) {
    ...
    return (
      <article
        ...
        data-orbit-root
        className={cn(
          "relative min-h-[280svh] overflow-hidden border-t border-[var(--border)]",
          className,
        )}
        aria-label={`${project.name} case study`}
      >
        {/* Morph stage — scales via transform, never layout props */}
        <div
          data-orbit-stage
        ...
        >
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse 65% 55% at 85% 35%, color-mix(in oklab, var(--accent) 18%, transparent), transparent 70%), linear-gradient(180deg, transparent 50%, var(--background) 100%)",
            }}
          />
          ...
          <div className="relative grid w-full gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
            ...
            {/* Abstract product frame — morphs with stage */}
            <div
              data-orbit-device
        ...
        <div className="relative space-y-28 px-0 pb-32 md:space-y-36">
          ...
          <div
            ...
            aria-label="Orbit architecture diagram"
          >
            <p className="font-mono text-[10px] tracking-[0.28em] text-[var(--accent)] uppercase">
              Diagram
            </p>
            <svg
              ...
              role="img"
            >
              <title>Orbit sync architecture</title>
              ...
              {project.architecture.nodes.map((node) => (
                <g key={node.id} data-orbit-node={node.id}>
                  <circle
                    cx={node.x}
                    cy={node.y}
                    r={18}
            ...
            {project.metrics.map((metric) => (
              <div key={metric.label}>
                ...
                <dd
                  ...
                  data-metric-value
                  data-metric-target={
                    metric.divisor ? metric.target / metric.divisor : metric.target
                  }
