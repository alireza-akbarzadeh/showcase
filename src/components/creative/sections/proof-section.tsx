      ...
      <div ref={ref} className={cn("w-full will-change-transform", className)}>
        ...
        <ul className="mt-16 grid gap-10 sm:grid-cols-3">
          {PROOF.siteMetrics.map((item, i) => (
            ...
            >
              <p className="font-mono text-[10px] tracking-[0.2em] text-[var(--muted-foreground)] uppercase">
                {item.label}
              </p>
              <p
                className="font-display mt-3 text-5xl tracking-[-0.04em] text-[var(--signal)] md:text-6xl"
                data-proof-value
              ...
              {item.note ? (
                <p className="mt-3 text-sm text-[var(--muted-foreground)]">{item.note}</p>
              ) : null}
            </li>
          ))}
        </ul>
        ...
        {orbit ? (
          ...
          >
            <div className="flex flex-wrap items-end justify-between gap-4 border-t border-[var(--border)] pt-10">
              ...
              <button
                type="button"
                className="font-mono text-[10px] tracking-[0.22em] text-[var(--muted-foreground)] uppercase underline-offset-4 hover:text-[var(--signal)] hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--signal)]"
            ...
            <div className="mt-12 grid gap-6 lg:grid-cols-2">
              {orbit.comparisons.map((cmp, i) => (
                ...
                >
                  <p className="border-b border-[var(--border)] px-5 py-3 font-mono text-[10px] tracking-[0.22em] text-[var(--muted-foreground)] uppercase">
                    {cmp.label}
                  </p>
                  <div className="relative grid min-h-[140px] grid-cols-2">
                    <div className="flex flex-col justify-end px-5 py-6">
                      ...
                      <span className="font-display mt-2 text-2xl tracking-[-0.02em] text-[var(--muted-foreground)] md:text-3xl">
                        {cmp.beforeValue}
                      </span>
                    </div>
                    <div className="flex flex-col justify-end border-l border-[var(--border)] bg-[color-mix(in_oklab,var(--signal)_8%,transparent)] px-5 py-6">
                      <span className="font-mono text-[10px] tracking-[0.2em] text-[var(--signal)] uppercase">
                        {cmp.afterLabel}
                      </span>
                    ...
                    <div
                      data-proof-wipe
                      className="pointer-events-none absolute inset-y-0 left-1/2 w-px origin-left bg-[var(--signal)] will-change-transform"
                      aria-hidden="true"
                    />
                  </div>
                  <p className="border-t border-[var(--border)] px-5 py-3 text-sm text-[var(--muted-foreground)]">
                    {cmp.detail}
                  </p>
                </div>
              ))}
            ...
            <dl className="mt-12 flex flex-wrap gap-10 md:gap-14">
              {orbit.metrics.map((metric) => (
                <div key={metric.id}>
                  ...
                  <dd
                    className="font-display mt-2 text-4xl tracking-[-0.03em] text-[var(--signal)] md:text-5xl"
        ...
        <p
          data-proof-closing
          className="mt-20 max-w-2xl border-t border-[var(--border)] pt-10 text-xl text-[var(--muted-foreground)] will-change-transform md:text-2xl"
        >
          {PROOF.closing}
        </p>
      </div>
